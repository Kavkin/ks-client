...
const FormConstructor: FC = (): JSX.Element => {
  ...
  function toggleCompSelection(compId: string | string[]): void {
    ...
    if (selectedCompIds.includes(compId)) {
      setSelectedCompIds((selectedCompIds) => selectedCompIds.filter((id) => id !== compId))
    } else {
      setSelectedCompIds((selectedCompIds) => [...new Set([...selectedCompIds, compId])])
    }
  }
  ...
  async function copySchema() {
    ...
    if (response.ok) {
      history.push(ROUTES.FORM_CONSTRUCTOR_EDIT.PATH.replace(':id', data.id))
    }
  }
  ...
  async function deleteSchema() {
    ...
    if (response.ok) {
      history.push(ROUTES.SCHEMA_LIST.PATH)
    }
  }
  ...
  return (
    <>
      ...
      />
      <Stack as="main" className="Main FormConstructor">
        <TreePanel
